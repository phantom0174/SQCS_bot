...
class Picture(Cog_Extension):
    ...
    async def p_m(self, ctx, *, msg):
        ...
        await getChannel(self.bot, '_Report').send(f'[Command]Group pic - p_m used by member {ctx.author.id}. {now_time_info("whole")}')
    ...
    async def p_check(self, ctx):
        ...
        await getChannel(self.bot, '_Report').send(f'[Command]Group pic - p_check used by member {ctx.author.id}. {now_time_info("whole")}')
    ...
    async def rpic(self, ctx):
        ...
        await getChannel(self.bot, '_Report').send(f'[Command]Group pic - rpic used by member {ctx.author.id}. {now_time_info("whole")}')
