...
class Mongo:
    ...
    def get_curs(self, collections: List[str]):
        cursors: List[pymongo.cursor.CursorType] = [
            self.client[collection] for collection in collections]
        return tuple(cursors)

    def get_all_curs(self):
        cursors: List[pymongo.cursor.CursorType] = [self.client[collection]
            for collection in self.client.list_collection_names()]
        return tuple(cursors)
